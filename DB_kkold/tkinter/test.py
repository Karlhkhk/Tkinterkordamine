...
# UUS: Funktsioon, mis uuendab andmed andmebaasis
def update_record(record_id, entries, window):
    # Võtame andmed sisestusväljadest
    new_values = (
        entries["Eesnimi"].get(),
...
def open_update_window(record_id):
    ...
    for i, label_text in enumerate(labels):
        ...
        entry.insert(0, record[i]) # Täida väli olemasoleva infoga
...
def on_update():
    ...
    if selected_item:
        record_id = selected_item[0] # iid, mille määrasime laadimisel
